   ...
   if(IsDraging)
   {
      ...
      if(DragedCard != null)
      {
         ...
         if(_loc1_ != null)
         {
            ...
            {
               Deck.Card = VoidCard;
               oldSocket = Cemetery.Socket;
               CardOnTop = false;
            }
            else
            {
               oldSocket = DragedCard.Socket;
            }
            if(IsFusionCard(_loc1_.Card))
            {
               if(_loc1_.Card.IsActive)
               {
                  if(BattleSystem["nFuseCard" + (_loc1_.Card.CardID - 600)] < BattleSystem["MAX_FUSE_CARD" + (_loc1_.Card.CardID - 600)])
                  {
                     if(IsTypeOfFusion(BattleSystem["nFuseTypeCard" + (_loc1_.Card.CardID - 600)],DragedCard))
                     {
                        BattleSystem["nFuseCard" + (_loc1_.Card.CardID - 600)]++;
                        DragedCard.Socket.Card = VoidCard;
                        var obj = BattleSystem.LookUpCard(DragedCard.CardID);
                        _loc3_ = BattleSystem.LookUpCard(_loc1_.Card.CardID);
                        _loc3_.Dice += obj.Dice;
                        _loc3_.Defence += obj.Defence;
                        setCardAttribute(_loc1_.Card,_loc3_.Dice,_loc3_.Defence);
                        Activate(DragedCard.Socket.Card);
                        DragedCard.removeMovieClip();
                     }
                  }
               }
            }
            else
            {
               ...
               {
                  DragedCard.Socket.Card.Socket = oldSocket;
   ...
   if(Math.abs(_loc1_.Socket._x - _loc1_._x) < 3)
   {
      if(Math.abs(_loc1_.Socket._y - _loc1_._y) < 3)
      {
         ...
         if(_loc1_.Type == DICE_RED)
         {
            ...
            BattleSystem.intervalID = setInterval(BattleSystem.CumulateRedDice,delayDiceMove);
         ...
         else if(_loc1_.Type == DICE_YELLOW)
         {
            ...
            {
               BattleSystem.intervalID = setInterval(BattleSystem.AIDispatchYellowDice,delayDiceMove);
               BattleSystem.Activate(_loc1_.Socket.LinkedCard);
               BattleSystem.LastActivatedCardSocket = _loc1_.Socket;
         ...
         else if(_loc1_.Type == DICE_GREEN)
         {
            if(BattleSystem.CurrentPlayer != BattleSystem.TURN_PLAYER)
            {
               BattleSystem.intervalID = setInterval(BattleSystem.AIDispatchGreenDice,delayDiceMove);
               BattleSystem.Activate(_loc1_.Socket.LinkedCard);
               BattleSystem.LastActivatedCardSocket = _loc1_.Socket;
               _loc1_.gotoAndStop("REMOVE");
            }
         ...
         else if(_loc1_.Type == DICE_BLUE)
         {
            if(BattleSystem.CurrentPlayer != BattleSystem.TURN_PLAYER)
            {
               BattleSystem.intervalID = setInterval(BattleSystem.AIDispatchBlueDice,delayDiceMove);
               BattleSystem.Activate(_loc1_.Socket.LinkedCard);
               BattleSystem.LastActivatedCardSocket = _loc1_.Socket;
               _loc1_.gotoAndStop("REMOVE");
            }
...
function GetStringCardIDInDeckWithCode(bMyDeck)
{
